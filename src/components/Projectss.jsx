...
const Projectss = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/projects/${departmentId}`);
  ...
  const handleAddProject = async () => {
    try {
      ...
      if (isEditing) {
        await axios.put(`http://localhost:5000/api/projects/${departmentId}/${project.id}`, newProject);
        setIsEditing(false);
      } else {
        await axios.post(`http://localhost:5000/api/projects/${departmentId}`, newProject);
      }
      ...
      <ul className="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
        {projects.map((project) => (
          ...
          >
            ...
            <button
              onClick={() => navigate(`/projectss/${departmentId}/${project._id}`, { state: { name: project.name, progress: project.progress } })}
