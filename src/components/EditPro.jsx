...
const EditPro = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/projects/${departmentId}/${projectId}`);
  ...
  const handleUpdateProject = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/projects/${departmentId}/${projectId}`, updatedProject);
